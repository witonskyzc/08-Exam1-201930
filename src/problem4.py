...
def problem4(point1, point2, n, window):
    ...
    for k in range(n+1):
        point = rg.Point(point1.x + (point2.x-point1.x)/(n*2+2)*(k+1), point1.y + (point2.y-point1.y)/(n*2+2)*(k+1))
        point.fill_color = point2.fill_color
        point.attach_to(window)

    for k in range(n+1):
        point = rg.Point(point2.x - (point2.x-point1.x)/(n*2+2)*(k+1), point2.y - (point2.y-point1.y)/(n*2+2)*(k+1))
